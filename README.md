...
| `open-webui/pipelines/factory_agent.py` | Dify SSE 프록시 Pipeline |
...
Pipeline은 Dify SSE 이벤트를 파싱하여 경로별로 다르게 처리합니다:

| Dify SSE 이벤트 | 경로 | Pipeline 동작 |
|-----------------|------|--------------|
| `message` | 일반 대화 (LLM) | 토큰 실시간 스트리밍 |
| `agent_message` | 간단한 조회 (Agent+MCP) | 토큰 실시간 스트리밍 |
| `node_started` (http-request) | 복잡한 분석 진입 | "🔍 분석 중..." 즉시 표시 |
| `message` (JSON) | LangGraph 응답 도착 | `response` 필드 추출 후 표시 |

> **conversation_id 자동 매핑**: Pipeline이 Open WebUI chat_id ↔ Dify conversation_id를 자동 변환합니다.
...
6. **진행 상태 표시** — Dify `node_started` 감지 시 즉시 "🔍 분석 중..." 표시
   - LangGraph 처리 5-10초 동안 사용자가 기다리지 않아도 됨
   - Stage 2에선 피드백 없이 대기
...
- **HTTP Request 노드 Timeout**: 반드시 **120초 이상**으로 설정
...
Stage 3: Pipeline → Dify 3분류 → "복잡한 분석" → "🔍 분석 중..."
         → LangGraph IntentAgent → defect_query 분류
...
│   └── pipelines/
│       └── factory_agent.py #   Dify SSE 프록시 Pipeline
│
├── docs/
